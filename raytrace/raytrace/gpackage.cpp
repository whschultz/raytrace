...
void GP_Object::GP_SetWorldCoordinates(float xMin,float xMax, float yMin, float yMax)
{  
...
void GP_Object::GP_AddPoints( float x, float y, int button )
{
	QString str;
	//  int spare;
	float *newpoints = 0;
	...
	switch (button)
    {
		case 0 :
			if (num_points == 0)
				points = new float[2];
			newpoints = new float[(2*num_points + 2)];
			//if (newpoints == 0)
		...
		case 1 :
			...
			if (tobedeleted != 0)
				...
				std::cout << "INVALID POINTER:  TOBEDELETED" << std::endl;
...
object::object(float input[],int verts)
{
...
void object::setobject(float input[],int verts)
{
...
void GP_Object::GP_DrawPixel(float x,float y,QColor color)
{
...
void object::drawobject(char *drawhere)
{
	...
	if (lred >255)
		std::cout << "lred " << lred << std::endl;
	if (lgreen >255)
		std::cout << "lgreen " << lgreen << std::endl;
	if (lblue >255)
		std::cout << "lblue " << lblue << std::endl;
	int same = 0;
...
{
	lobject *sparel;
	float vertices[4] = {a.x(),a.y(),b.x(),b.y()};
	if (shapes == 0)
    {
		shapes = new lobject;
...
int object::proximity(vertex v,float distance)
{
	int where = -1;
	int q=0;
	float d;
...
int lobject::proximity(vertex v,float distance,int *a)
{
	int where = -1;
	int q=-1;
	lobject *spare = this;
...
int GP_Object::GP_Proximity(float x,float y,int distance,int *a)
{
	if (shapes == 0)
		return -1;
	vertex thepoint(x,y);
...
int GP_Object::GP_Proximity(float x,float y,int distance)
{
	int useless;
	if (shapes == 0)
		return -1;
...
void GP_Object::GP_Polygon(float *data,int num_verts)
{
	lobject *sparel;
	if (shapes == 0)
    {
