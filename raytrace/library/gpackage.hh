...
{
public:
  vertex(double a,double b) {x_coor=a; y_coor=b;};
  vertex(void) { };
  ...
  friend vertex operator/(vertex a,vertex b) {
    vertex c(a.x_coor / b.x_coor,a.y_coor / b.y_coor); return c;};
  void set(double a,double b) {x_coor=a; y_coor=b;};
  double x(void) {return x_coor;};
  double y(void) {return y_coor;};
  //double distance(vertex v) {double d=pow(pow(x_coor - v.x_coor,2)+pow(y_coor-v.y_coor,2),.5);return d;}
  double distance(vertex v) {return hypot(x_coor-v.x_coor,y_coor-v.y_coor);}
private:
...
{
public:
  object(double [],int); //this int represents the number of vertices, not the size of the array
  object(void) { }
  ~object() {delete [] vertices;};
  void setobject(double*,int);
  ...
  int proximity(vertex,double);  //  point to compare at what distance
private:
  vertex *vertices;
  int number;
...
public:
  ...
  void set(double *data, int num, lobject *pd, lobject *nd) 
     {theShape = new object(data,num); next = nd; previous = pd;};
  ~lobject();
  void drawobjects(char *);
  int proximity(vertex,double,int *);
};
...
public:
  ...
  void GP_AddPoints(double,double,int);
  void GP_Initialize();
  void GP_Terminate();
  void GP_Line(vertex,vertex);
  void GP_Polygon(double[],int);
  ...
  void GP_SetWorldCoordinates(double,double,double,double); //xMin,xMax,yMin,yMax
  int  GP_Proximity(double,double,int,int*);
  int  GP_Proximity(double,double,int);
  ...
  void GP_DrawPixel(double,double,QColor);
